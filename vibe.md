...
- 現在代碼/系統是否已優化好，沒有測試代碼及實驗性代碼，穩定而且可以應用在生產環境?
- 保留現在的功能與UI呈現效果，不要使用實驗性代碼，嘗試進一步優化，整合，簡化代碼，提高可維護性，避免代碼的結構過於複雜，以及處理所有潛在問題,解決所有曾出現的編程問題
- 整個系統,不要使用實驗性代碼，嘗試進一步優化，整合，簡化代碼，以及處理所有潛在問題,解決所有曾出現的編程問題,完整性檢查,修復,完善
- 刪去已不使用，過時，舊版，冗餘，無用及非系統相關的function及代碼，以優化簡化系統，提升可維護性
...
- 不要破壞性修改
- 可以完全廢除之前的所以版本，不再需要兼容，允許本次破壞性地重構，以提供優化空間給最新版本可以更容易地簡化及優化，但仍需要設計時考慮未來的兼容性
- 每組代碼需要有marking，完整檢查有沒有遺漏，以防後期版本控制出錯
- 下一輪的深入檢查
- 允許使用上網搜索原因及解決方法
- 完整性檢測，安全性檢測(身份驗證,ouath,權限,漏洞，邏輯錯誤及異常檢測)，舊代碼,重疊代碼優化,簡化,統一,修復,清理
- 完整性檢查，掃瞄整個系統所有代碼,精簡複雜代碼及刪除多餘文件及代碼,不要使用實驗性代碼及過度優化,同時保守策略修改系統代碼，嘗試進一步優化，整合，簡化代碼，以及處理所有潛在問題,解決所有曾出現的編程問題，檢查，修復，完善代碼
- 是否已經簡化及優化好?而且在應用層層面上是可以推出到生產環境?
- database 仍然暫用sqlite, api gateway會另外設置,redis也是另外設定,https相關也是在之後進行,目前只針對應用層
- 在功能不變情況下進行大規模以簡化為本去優化，注意不要過度優化，清理無用及多餘及舊代碼，在功能不變情況下以簡化為本去整合，處理所有潛在，編程及過去未修復的異常問題。
- 以簡化為本,功能不變,不要過度地優化，刪去不再需要的代碼及文件,以及無用組件,舊版與廢棄的代碼等等
- 分析整個系統,包括前端app及後端 , 在功能不變,不要過度優化情況下是否有需要進行大規模以簡化為本的優化，去複雜化(包括清理無用及多餘及舊代碼，在功能不變情況下以簡化為本去整合，處理所有潛在，編程及過去未修復的異常問題。)
- 深度完整檢查,以及檢查安全,權限,漏洞
- 盡可能安全無漏洞地，核心功能不變，以簡化為本去優化，以及修復代碼修復編譯問題等等，令系統更輕巧而且穩定
- 在功能不變,不要過度優化情況下以簡化為本的優化，去複雜化(包括清理無用及多餘及舊代碼，在功能不變情況下以簡化為本去整合，處理所有潛在，編程及過去未修復的異常問題，以及安全，漏洞與權限問題。)
- 深度去複雜化,解決歷史遺留的問題,去除技術債,保留功能,修復安全,權限,漏洞,編程及過去未修復的異常問題
- 為防之後因代碼混淆而令ai改錯代碼,請保留功能下去複雜化,去多餘代碼及文件,包括前後端
- 在保留功能下去深度去複雜化要精簡化,解決歷史遺留的問題,去除技術債及死代碼與無用文件,修復安全,權限,漏洞,編程及過去未修復的異常問題
---
